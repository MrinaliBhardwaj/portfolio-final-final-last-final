...
## A project page is its UI screens (2 Sep 2026)

Her call: the pages after the profile carry the UI screens from that project
and nothing else. Three source files named — Regis, NextG Apex, Layover — and
a horizontal scroll "where required".

**The rail aligns by HEIGHT, not width.** That is the whole reason it works: a
set of UI screens is a mix of portrait phones, tablets and full-bleed desktop
boards, and only a shared height makes them read as one sequence rather than a
ransom note. Width follows from each screen's own aspect. NextG proves it —
613px desktop, 909px coverage map, four 188px phones, all exactly 420 tall,
none cropped and none letterboxed.

**`screensAs: "rail"` opts a project in.** The grid stays for projects whose
work is wide composed boards, which a rail would shrink to postcards.

**Two new projects.** Regis was a README only; it now has twelve screens and a
design entry, and the README stays as the engineering view of the same work.
NextG Apex is new. That makes five projects in the sidebar — which is what the
reference showed.

**Figma has two doors and only one opens.** The `Figma` MCP in this session is
the desktop Dev Mode bridge and reports that the Dev Mode MCP Server is not
enabled. The remote connector reads figma.com by fileKey + nodeId and works:
`get_metadata` to enumerate frames, `download_assets` at 2x, then WebP.

**The Layover file is blocked on a server bug**, not on access. `get_metadata`
fails identically every time — `Failed to parse SSE message ... EOF while
parsing a string at line 1 column 137971` — so its child frames cannot be
enumerated, and `get_design_context` on that file falls through to the desktop
selection path. `get_screenshot` works, which is how we know node 204:1139 is
"Every screen used in the case study, at full size, grouped by surface" at
6060 × 24319. Unblocking it needs node-specific links (right-click a frame →
Copy link to selection). Layover keeps its five existing boards in a rail
meanwhile.

**Still 4 folder slots on the desk for 5 case studies** — flagged, not solved,
because which four sit on the desk is hers to decide.
